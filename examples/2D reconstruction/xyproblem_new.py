    ...
    if args.viz:
        ...
        with torch.no_grad():
            latents = model.encoder(
                torch.cat([va_ds.data, va_ds.dist], dim=1).to(device)
            ).cpu().numpy()
            labels = va_ds.labels.squeeze().numpy()
        emb = TSNE(n_components=2, init="random", perplexity=30).fit_transform(latents)
        plt.figure(figsize=(6, 5))
        plt.scatter(emb[:, 0], emb[:, 1], c=labels, cmap="coolwarm", alpha=0.6)
        plt.title("t‑SNE of full latent")
        plt.tight_layout()
        plt.savefig("tsne_latent.png", dpi=150)
        print("t‑SNE saved to tsne_latent.png")
        """
